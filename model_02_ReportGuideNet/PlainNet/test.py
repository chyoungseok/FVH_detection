...
from datasets.flair2d_binary import FlairH5SliceDataset, compute_pos_weight_from_dataset
...
from utils.common import seed_everything, is_main_process
from cam.gradcam import GradCAM, overlay_cam_on_image
...
def main(args):
    ...
    if is_gradCam:
        # --- Grad-CAM ---
        target_layer = model.backbone.layer4[-1]   # resnet 기준
        cam_extractor = GradCAM(model, target_layer)

        out_cam_dir = os.path.join(cfg["out"]["output_dir"], "cam_images")
        os.makedirs(out_cam_dir, exist_ok=True)

        model.eval()
        for batch in tqdm(test_loader, desc="Grad-CAM saving"):  # ✅ tqdm progress bar
            ...
            cams = cam_extractor.generate(outputs)

            for i in range(len(imgs)):
                img_np = imgs[i][0].detach().cpu().numpy()   # (H,W)
                cam = cams[i]
                overlay = overlay_cam_on_image(img_np, cam)

                sid = sids[i]
                prob = float(preds[i].detach().cpu().item())
                true_label = int(labels[i].cpu().item())
                pred_label = int(prob >= 0.5)

                # 파일명에 sid, true y, pred y(prob) 포함
                filename = f"{sid}_slice{slice_idxs[i]:03d}_true{true_label}_pred{pred_label}_prob{prob:.3f}.png"
                out_path = os.path.join(out_cam_dir, filename)

                plt.figure(figsize=(4, 4))
                plt.imshow(overlay)
                plt.axis("off")
                plt.title(f"{sid} | prob={prob:.3f} | true={true_label} | pred={pred_label}")
                plt.savefig(out_path, bbox_inches="tight")
                plt.close()
        
    if not is_gradCam and mcfg.get("swin", {}).get("enabled", False):
        from cam.attention_rollout import AttentionRollout

        out_cam_dir = os.path.join(cfg["out"]["output_dir"], "attn_rollout_images")
        os.makedirs(out_cam_dir, exist_ok=True)

        rollout = AttentionRollout(model, head_fusion="mean")
        model.eval()

        for batch in tqdm(test_loader, desc="Attention Rollout saving"):
            imgs = batch["image"].to(device)  # (B,1,H,W)
            labels = batch["label"].to(device)
            sids = batch["sid"]
            slice_idxs = batch["slice_idx"]
            ...
            for i in range(len(imgs)):
                ...
                overlay = cv2.addWeighted(
                    cv2.cvtColor((img_np*255).astype(np.uint8), cv2.COLOR_GRAY2BGR),
                    0.5, heatmap, 0.5, 0
                )

                sid = sids[i]
                prob = float(torch.sigmoid(model(imgs[i:i+1])).cpu().item())
                true_label = int(labels[i].cpu().item())
                pred_label = int(prob >= 0.5)
    ...
    # if not is_gradCam and mcfg.get("swin", {}).get("enabled", False):
    #     from cam.swin_cam import swin_gradcam_visualize

    #     out_cam_dir = os.path.join(cfg["out"]["output_dir"], "swin_cam_images")
    #     swin_gradcam_visualize(model, test_loader, device, out_cam_dir)
